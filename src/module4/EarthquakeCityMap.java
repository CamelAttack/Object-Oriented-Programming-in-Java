...
public class EarthquakeCityMap extends PApplet {
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			// Use isInCountry method to check if an earthquake is on land
			// Also will assign the country to that PointFeature
			if (isInCountry(earthquake, m)) {
				// The earthquake is on land return true
				return true;
			}
		}
	...
	private void printQuakes() {
		...
		// Create a hash map for each countries quake counter
		// Uses the country name as a key
		HashMap<String, Integer> quakeCounterMap = new HashMap<String, Integer>();
		// Create a counter for ocean earthquakes
		int oceanQuakeCounter = 0;
		// Loop through all the country markers to save their names as the hash map keys
		for (Marker cm : countryMarkers) {
			String name = (String) cm.getProperty("name");
			quakeCounterMap.put(name, 0);
		}

		// Loop through all of the earthquakes to count how many occur in each country
		// Or occur in the ocean
		for (int i = 0; i < quakeMarkers.size(); i++) {
			EarthquakeMarker quakeMarker = (EarthquakeMarker) quakeMarkers.get(i);
			if (quakeMarker.isOnLand) {
				// Get the name of the country the earthquake occurred in
				String countryLocation = (String) quakeMarker.getProperty("country");
				// Increment that countries counter in the hash map
				int counter = quakeCounterMap.get(countryLocation);
				counter++;
				quakeCounterMap.put(countryLocation, counter);
			} else {
				// The earthquake occurred in the ocean so increment the ocean counter.
				oceanQuakeCounter++;
			}
		}

		// Print each country and the number of quakes that occurred there
		// Or skip that country if no quake occurred there
		Set counterSet = quakeCounterMap.entrySet();
		Iterator counterIterator = counterSet.iterator();
		while (counterIterator.hasNext()) {
			HashMap.Entry<String, Integer> counterEntry = (HashMap.Entry) counterIterator.next();
			int countNumber = (int) counterEntry.getValue();
			if (countNumber > 0) {
				System.out.println(counterEntry.getKey() + ": " + countNumber);
			}
		}
		System.out.println("Oceans: " + oceanQuakeCounter);
	}
